...
DROP POLICY IF EXISTS "Users can only see their own prompt views" ON public.prompt_views;
DROP POLICY IF EXISTS "Users can only see their own views" ON public.prompt_views;

-- Create a new policy that only allows users to see their own views or anonymous views
CREATE POLICY "Users can only see their own views"
ON public.prompt_views
FOR SELECT
USING (auth.uid() = user_id OR user_id IS NULL);

-- Drop existing policies on ad_revenue if they exist
DROP POLICY IF EXISTS "Users can view their own ad revenue" ON public.ad_revenue;

-- Create ad_revenue table if it doesn't exist
...
-- Drop existing policies on withdrawal_requests if they exist
